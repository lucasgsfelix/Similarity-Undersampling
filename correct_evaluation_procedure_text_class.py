...
if __name__ == '__main__':
	...
	for fold in tqdm.tqdm(folds_index.items()):
		...
		fold_x_train, fold_y_train = x_train.iloc[folds_index[fold]['train']], y_train.iloc[folds_index[fold]['train']]
		...
		for func_used in ['head', 'tail']:
			...
			for amount_instances in tqdm.tqdm([1000, 5000, 10000, 20000, 30000, 40000, 50000, 75000, 100000,
				...
				for balanced in [True, False]:
					...
					prediction = model.predict(x_train.iloc[folds_index[fold]['test']])

					y_test_fold = x_train.iloc[x_train.iloc[folds_index[fold]['test']]]
